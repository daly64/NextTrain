...
class TrainInterval {
    ...
    val tunis_to_rades = listOf<String>(
        "05:00", "05:30",
    ...
    val rades_to_tunis = listOf<String>(
        "04:59", "05:30",
    ...
    fun radesToTunis(label: TextView) {
        getTrainRemainTime(label, rades_to_tunis, "Tunis")
    }

    fun tunisToRades(label: TextView) {
        getTrainRemainTime(label, tunis_to_rades, "Rades")
    }

    private fun getTrainRemainTime(label: TextView, tab: List<String>, destinationTown: String) {
        var i = 0
...
//                parse train to time format
                val date = LocalTime.parse(t)
//                if train time is after current time
                if (date.isAfter(time)) {
                    val h = time.until(date, ChronoUnit.SECONDS) / 3600
                    val mn = time.until(date, ChronoUnit.SECONDS) % 3600 / 60
                    val s = time.until(date, ChronoUnit.SECONDS) % 3600 % 60
